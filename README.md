...
- [x] **6. Substituir mock data do dashboard por dados reais e gráficos interativos.**
  ...
  > *Resolvido.* `PatientDashboard.tsx` completamente reescrito: (1) **4 metric cards** — Active Medications, Adherence Rate, Doses Taken, Doses Missed (dados reais de `GET /medications` + `GET /medications/adherence/stats`); (2) **Upcoming Doses** — dados reais de `GET /medications?limit=5&status=active` com empty-state e link "Add medication"; (3) **Weekly Adherence chart** — `recharts` `<BarChart>` com cores condicionais (verde ≥90%, amarelo ≥70%, vermelho <70%) alimentado por `GET /medications/schedule`. Zero mock data. 3 chamadas API em paralelo via `Promise.allSettled`. [`inserir-hash-commit`](https://github.com/ds-sv/caresync/commits/frontend-overhaul/)
